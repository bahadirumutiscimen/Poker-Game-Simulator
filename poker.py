...
def simulate_hands(deck, hands):
  """
  Sinulate multiple hands of cards
  """
  results = []
...
def play_poker_hand(hand1, hand2):
    ...
    # print the hands as well as the rank adn type of hand
...
@cli.command("info")
@click.option("--probability", is_flag=True, help="Display the probabality of each hand of poker")
...
@click.option("rounds", "--round", default=1, help="Number of rounds to play")
...
def interactive(name, rounds, money):
    ...
    for i in range(1, rounds + 1):
        ...
        history["probability_of_hand"][f"rond{i}"] = hand_probability
        expected_value = (1 - (hand_probability * 0.001)) * abs(bet)
